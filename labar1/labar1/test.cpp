...
inline void check_equal_matrix(MATRIX& m1, MATRIX& m2, function<double(double)> f)
{
    for (int row = 0; row < ROWS; ++row)
        for (int col = 0; col < COLS; ++col)
            check_equal(m2.GetMij(row, col), f(m1.GetMij(row, col)));
...
inline void check_equal_matrix(MATRIX& m1, MATRIX& m2, MATRIX& m3, function<double(double, double)> f)
{
    for (int row = 0; row < ROWS; ++row)
        for (int col = 0; col < COLS; ++col)
            check_equal(m2.GetMij(row, col), f(m1.GetMij(row, col), m3.GetMij(row, col)));
...
void test_time2()
{
    ...
    for (int i = 0; i < ROWS * ROWS; ++i)
    {
        ...
        {
            m1.SetMij(i, j, (rand() % 100) / 100.0);
            m2.SetMij(i, j, (rand() % 100) / 100.0);
        }
    }
